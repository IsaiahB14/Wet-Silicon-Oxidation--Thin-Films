...
x_final(T, t) = x_physics(T, t) + x_GP(T, t)

7. Streamlit GUI

A simple Streamlit GUI was also created so the user can interact with the model more easily. This GUI is where the user will enter their desired thickness or time and temperature values to get the program’s output. When using this code, this is where the user will spend most of their time.

Once the folder is opened in Python, run the following command in the terminal:

streamlit run scripts/GUI_Predictor.py

Ensure Streamlit is installed. If it is not installed, install it using:
